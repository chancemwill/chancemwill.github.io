...
const data_url =
  "https://raw.githubusercontent.com/chancemwill/Glasgow-Tramways/main/Glasgow-Corporation-Tramways.geojson";
map.on("load", () => {
  map.addLayer({
    id: "Glasgow-Trams",
    type: "line",
  ...
  document.getElementById("slider").addEventListener("input", (event) => {
    ...
    if (max == 1) {
      display.innerText =
        "3 May 1948: Route numbering system introduced. Region wide routes numbered 1 - 21. Routes 30 and 31 introduced as temporary routes for 1938 Empire Exhibition.";
    }
    if (max == 2) {
    ...
    if (max == 20) {
      display.innerText = "8 April 1945: Route 30 extended to Cambuslang.";
    }
    if (max == 21) {
      display.innerText =
    ...
    if (max == 25) {
      display.innerText =
        "17 March 1946: Route 34 introduced. Route 22 extended to Lambhill. Route 31 replaced by new services by routes 27 and 32.";
    }
    if (max == 26) {
      display.innerText =
    ...
    if (max == 35) {
      display.innerText =
        "29 May 1949: Route 34 new service pattern between Auchenshuggle and Alexandra Park. Route 35 ends.";
    }
    if (max == 36) {
      display.innerText =
        "31 July 1949: Route 31 extended to Canal Bridge, Blairdardie.";
    }
    if (max == 37) {
    ...
    if (max == 38) {
      display.innerText =
        "4 December 1949: Route 20 ends, replaced by bus service 20.";
    }
    if (max == 39) {
      display.innerText =
    ...
    if (max == 47) {
      display.innerText =
        "2 November 1952: Route 31 terminus moved to Nether Auldhouse Rd.";
    }
    if (max == 48) {
      display.innerText =
    ...
    if (max == 52) {
      display.innerText = "4 November 1953: Route 13 replaced by route 29.";
    }
    if (max == 53) {
      display.innerText =
        "7 February 1954: Route 4a extended to north Paisley. Route 40 terminus moved to Garscube Gate.";
    }
    if (max == 54) {
      display.innerText =
    ...
    if (max == 59) {
      display.innerText =
        "3 November 1956: Route 15 service to Airdrie, Route 17 service to Cambuslang, and Route 29 service to Milngavie ends. Airdrie local route discontinued.";
    }
    if (max == 60) {
      display.innerText =
    ...
    if (max == 66) {
      display.innerText =
        "15 November 1958: Tram routes 12, 17, 22, 26, 32 end. Route 26 reorganised.";
    }
    if (max == 67) {
      display.innerText =
  ...
  map.on("click", "Glasgow-Trams", (event) => {
    ...
    new mapboxgl.Popup({
      ...
      <h6>
      ${feature.properties.Start}</h6>
      <p>
      ${feature.properties.Comment_1}</p>
      <h6>
      ${feature.properties.End}</h6>
      <p>
      ${feature.properties.Comment_2}</p>`
      )
      .addTo(map);
  });
